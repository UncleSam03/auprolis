...
const AdminUsers = () => {
  const users = [
    { name: 'Lesedi Moloi', email: 'l.moloi@fnb.bw', role: 'Agent', institution: 'FNB Botswana', plan: 'Enterprise', price: '$45/mo', date: 'Oct 12, 2023', status: 'Active', initials: 'LM', color: 'primary' },
    { name: 'Thabo Ntsite', email: 'thabo.n@property.co.bw', role: 'Seller', institution: '—', plan: 'Pro', price: '$30/mo', date: 'Nov 04, 2023', status: 'Active', initials: 'TN', color: 'secondary' },
    { name: 'Kabelo Kebone', email: 'kabelo.k@outlook.com', role: 'Buyer', institution: '—', plan: 'Basic', price: '$15/mo', date: 'Jan 15, 2024', status: 'Pending', initials: 'KK', color: 'error' },
    { name: 'Sarah Moabi', email: 'sarah@stanbic.bw', role: 'Admin', institution: 'Stanbic Bank', plan: 'Enterprise', price: '$45/mo', date: 'Dec 20, 2023', status: 'Suspended', initials: 'SM', color: 'tertiary' },
  ];
    ...
    <AdminDashboardLayout title="Community Management" subtitle="Monitor and verify user accounts across the Botswana real estate ecosystem.">
      <div className="space-y-12 pb-24">
                ...
                <tr key={idx} className="group hover:bg-surface transition-all duration-300">
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-6">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-[900] text-sm font-headline shadow-inner scale-100 group-hover:scale-110 transition-transform ${
                        user.color === 'primary' ? 'bg-primary/10 text-primary' : 
                        user.color === 'secondary' ? 'bg-secondary/10 text-secondary' : 
                        user.color === 'error' ? 'bg-error/10 text-error' : 'bg-tertiary/10 text-tertiary'
                      }`}>
                        {user.initials}
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="font-[800] text-[1.1rem] text-on-surface font-headline leading-tight group-hover:text-primary transition-colors">{user.name}</span>
                  ...
                  <td className="px-8 py-8">
                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest leading-none shadow-sm ${
                      user.role === 'Agent' ? 'bg-tertiary/10 text-tertiary' : 
                      user.role === 'Seller' ? 'bg-primary/10 text-primary' : 
                      user.role === 'Admin' ? 'bg-on-surface/90 text-white' : 'bg-surface-container-high text-on-surface-variant opacity-60'
                    }`}>
                      {user.role}
                    </span>
                  </td>
                  ...
                  <td className="px-8 py-8">
                    <div className="flex flex-col gap-1">
                      <span className="text-[13px] font-[900] text-on-surface leading-none">{user.plan}</span>
                      <span className="text-[9px] font-black uppercase tracking-[0.1em] text-outline/40 leading-none">{user.price}</span>
                    </div>
                  </td>
                  <td className="px-8 py-8 text-xs font-bold text-on-surface-variant opacity-60 font-body">{user.date}</td>
                  <td className="px-8 py-8">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-2.5 h-2.5 rounded-full shadow-lg ${
                        user.status === 'Active' ? 'bg-emerald-500 shadow-emerald-500/20' : 
                        user.status === 'Pending' ? 'bg-amber-400 shadow-amber-400/20' : 'bg-error shadow-error/20'
                      }`}></div>
                      <span className={`text-[10px] font-black uppercase tracking-widest ${
                        user.status === 'Active' ? 'text-emerald-700' : 
                        user.status === 'Pending' ? 'text-amber-700' : 'text-error'
                      }`}>
                        {user.status}
                      </span>
                    </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {[
            { label: 'Total Institutional Users', value: '2,104', trend: '+12%', color: 'emerald', icon: 'corporate_fare' },
            { label: 'Pending Approvals', value: '43', icon: 'hourglass_empty', color: 'amber' },
            { label: 'Platform Revenue', value: 'P842.2k', trend: '+5%', color: 'emerald', icon: 'account_balance' },
            { label: 'Network Gravity', value: '99.9%', subtext: 'System Stable', icon: 'shield', color: 'primary' }
